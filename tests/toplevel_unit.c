...
void sbs_compare(sbs_t *sbs) {
    ...
    uint32_t *expected_values = typed_malloc_n(uint32_t, expected_cardinality);
    ...
    uint32_t *actual_values = typed_malloc_n(uint32_t, actual_cardinality);
...
void test_iterator_generate_data(uint32_t **values_out, uint32_t *count_out) {
    ...
    uint32_t *values = typed_malloc_n(uint32_t, capacity);  // ascending order
    uint32_t count = 0;
